...
# 
### Visualisations focusing on care for older adults:
...
::: {.fragment .fade-in}
Residental social care is by far the biggest catergory.
:::

#
## Visualising the present scope and quantity of CQC services {.scrollable}

Using the current CQC data release and R:

1.  Filtered the data for active services.
2.  Grouped the locations by their Primary Care Catergory and made a tally.
3.  Then plotted the totals for each catergory as an ordered barplot.
...
#  {background-image="img/cqc-scope-2022.png" background-size="90%"}
...
## Visualising the change in care home bed capacity {.scrollable}

I filtered CQC data from 2013 to 2022 for care homes for older people, and then
created a combined 2013 to 2022 table.
...
#  {background-iframe="img/res_beds_yrs.html" background-interactive="true" margin="0.2"}

## Care support requests for older people is stable in the short term, but with large regional variation {.center background-color="#005C84"}

## Visualising care requests {.scrollable}
...
2. I then calculated the total number of requests for each region
and scaled the requests per 1,000 people 65+ in each region using 2020 estimates.
...
#  {background-image="img/care-requests-2018-2022.png" background-size="80%"}

## Care quality is stable across all regions, and most care homes are rated Good {.center background-color="#005C84"}

## Visualising care quality {.scrollable}



Using CQC data from 2016-2022, I created a filter for residential care homes to filter
the yearly tables.
...
#  {background-iframe="img/cqc_quality_yrs.html" background-interactive="true"}

## Visualising Dementia Care Homes on a map
...
##  {background-iframe="img/cqc_dementia_homes_map.html" background-interactive="true"}
